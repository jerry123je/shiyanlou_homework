...
class UserForm(FlaskForm):
    username = StringField('user name', validators=[Required(), Length(4,32)])
    email = TextAreaField('user intro', validators=[Required(), Length(20,256)])   
    password = PasswordField('密码', validators=[Required(), Length(6, 24)])
    repeat_password = PasswordField('重复密码', validators=[Required(), EqualTo('password')])
    role = IntegerField('role id', validators=[Required(), NumberRange(min=1, message='invalid user role')])
    job = StringField('job', validators=[Required(), Length(4,32)])
    submit = SubmitField('提交') 
